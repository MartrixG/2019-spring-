...
class RemoveComments {
    public List<String> removeComments(String[] source) {
        ...
        for (String line: source) {
            ...
            while (i < line.length()) {
                if (!inBlock && i+1 < line.length() && chars[i] == '/' && chars[i+1] == '*') {// Out of index
                    inBlock = true;
                } else if (inBlock && i+1 < line.length() && chars[i] == '*' && chars[i+1] == '/') {// Out of index
                    inBlock = false;
                } else if (!inBlock && i+1 < line.length() && chars[i] == '/' && chars[i+1] == '/') {// Lack of //
                    inLine = true;
